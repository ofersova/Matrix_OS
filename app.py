...
# --- פונקציות שאיבת נתונים עם זיכרון מטמון מובנה ---
...
st.markdown("""
    ...
    /* מניעת שקיפות סטרימליט לחלוטין ללא דריסת הרקע */
...
# --- רשימות נכסים קבועות מחוץ ללולאה (לחיסכון בזיכרון) ---
assets = {
...
# =====================================================================
# === מנוע הדשבורד החי (טוען את עצמו שקטים בכל 15 שניות, ללא הבהוב) ===
# =====================================================================
...
def live_dashboard():
    ...
    st.write(f"🔄 מתעדכן חי (ללא הבהוב מסך) | זמן מערכת: {now_dt.strftime('%H:%M:%S')}")
    st.markdown("---")
    ...
    with col_m3:
        ...
        st.info("💡 שים לב: המערכת פועלת כעת בתצורה שקטה (Fragments) למניעת הבהובים.")
        ...
        try:
            ...
            if not df_sector.empty:
                ...
                if ticker == 'XLF' and tnx_val > 4.2: 
                    ...
                    lead_reason, is_macro_aligned = "נפט ב-Backwardation", (confluence_score > 0)
                ...
                if confluence_score > 25: status_text, trigger_text = f"🟢 +{confluence_score:.1f} (לונג)", f"{info['long_3x']}"
                elif confluence_score < -25: status_text, trigger_text = f"🔴 {confluence_score:.1f} (שורט)", f"{info['short_3x']}"
    ...
    if matrix_table_data:
        ...
        # סידור העמודות בדיוק מוחלט (עם הכותרות הרחבות המקוריות והתקינות)
        df_matrix = df_matrix[[
            'פאנל חיווי',
            'הדק (ביצוע)',
            'סקטור (בסיס)',
...
# הרצת הפונקציה (ללא שום פקודת Sleep או Rerun - הכל מתנהל אוטומטית מאחורי הקלעים!)
live_dashboard()

# הערה קטנה: אם השרת מציג במקרה שגיאה אדומה שקשורה למילה 'fragment',
# פשוט כנס לקובץ requirements.txt והחלף את המילה streamlit במילים:
# streamlit>=1.37.0
